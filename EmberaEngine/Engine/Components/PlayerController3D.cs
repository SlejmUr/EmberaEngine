...
class PlayerController3D : Component
{
    ...
    private void HandleCameraLook(float dt)
    {
        ...
        yaw += mouseDelta.Y * mouseSensitivity;
        pitch -= mouseDelta.X * mouseSensitivity;

        yaw = Math.Clamp(yaw, -89f, 89f);

        // Apply rotation
        camera.gameObject.transform.Rotation = new Vector3(pitch, yaw, 0f);
    }
